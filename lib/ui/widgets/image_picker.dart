...
class ImagePickerContainer extends StatefulWidget {
  final Widget child;
  final Function(BuildContext context, File? image, List<File>? images) onPick;
          ...
          children: <Widget>[
            TextButton(
              onPressed: () async {
                ...
                widget.onPick(context, file, null);
            ...
            TextButton(
              onPressed: () async {
                if (ImagePickerType.photo == widget.type) {
                  var file = await ImagePickerContainer.getImage(
                    context,
                    ImageSource.gallery,
                  );
                  widget.onPick(context, file, null);
                } else if (ImagePickerType.multiplePhoto == widget.type) {
                  var files = await ImagePickerContainer.getImages(context);
                  widget.onPick(context, null, files);
                }
                Navigator.pop(context);
              },
